...
/-!
The `rebuild_luebeckConwayPolynomial?` command renders a coefficient-table
excerpt for inspection from cached Lübeck data. The complete committed library
is regenerated by `scripts/conway/generate.py`, including both proof tiers.
...
`scripts/conway/import_source.py`, which is the only step that
...
or does not have the expansion input’s JSON shape, since the
...
them. The complete offline generator instead accepts an exact pair list.
